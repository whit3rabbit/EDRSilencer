...
namespace bof {
    ...
    namespace mock {
        ...
        void BofData::insert(const std::vector<char>& data) {
            pack<int32_t>(static_cast<int32_t>(data.size()));
            append(data);
        }
            ...
            if (stage.module.empty()) {
                ...
                switch (stage.allocator) {
                    ...
                    case bof::profile::Allocator::MapViewOfFile: {
                        hFile = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_EXECUTE_READWRITE, 0, static_cast<DWORD>(size), NULL);
    ...
    void runMockedSleepMask(char* mask, int len, int sleep_time, void* beacon_entry) {
        BeaconPrintf(CALLBACK_OUTPUT, "[+] Mocked sleep mask is running with sleep time: %d ms.\n", sleep_time);
    }
}
...
namespace mock {
namespace syscall {
    // Stubs for functions that would be in the missing mock_syscalls.cpp
    // NOTE: This is a placeholder. You will need a real implementation
    // to test functionality that relies on BeaconGetSyscallInformation.
...
extern "C"
{
    ...
    void BeaconFormatPrintf(formatp* format, const char* fmt, ...) {
        va_list args;
        va_start(args, fmt);
        int size = vsnprintf(format->buffer, format->length, fmt, args);
        if (size > 0) {
            format->buffer += size;
            format->length -= size;
        }
    ...
    void BeaconFormatAppendW(formatp* format, const wchar_t* text, int len) {
        BeaconFormatAppend(format, (const char*)text, len * sizeof(wchar_t));
    }
