...
def generate_c_code():
    ...
    print('#include "utils.h" // For BOOL definition')
    print('#include <stdlib.h>')
    print('#include <string.h>')
    print('')
    print(f'const char XOR_KEY = 0x{XOR_KEY:02x};')
    print('')
    ...
    print('')
    print('static BOOL itemProcessed[sizeof(processData) / sizeof(processData[0])] = { FALSE };')
    print('')
    print('// This function is defined here as it is only used by isProcessInList')
    print('char* decryptString(struct EncryptedString encStr) {')
    print('    if (!encStr.data || encStr.size == 0) return NULL;')
    print('    char* decrypted = (char*)malloc(encStr.size + 1);')
    print('    if (!decrypted) return NULL;')
    print('    for (size_t i = 0; i < encStr.size; ++i) {')
    print('        decrypted[i] = encStr.data[i] ^ XOR_KEY;')
    print('    }')
    print('    decrypted[encStr.size] = \'\\0\';')
    print('    return decrypted;')
    print('}')
    print('')
    print('BOOL isProcessInList(const char* procName) {')
    print('    for (size_t i = 0; i < PROCESS_DATA_COUNT; i++) {')
    print('        char* decryptedName = decryptString(processData[i]);')
    print('        if (!decryptedName) continue;')
    print('        if (_stricmp(procName, decryptedName) == 0 && !itemProcessed[i]) {')
    print('            itemProcessed[i] = TRUE;')
    print('            free(decryptedName);')
    print('            return TRUE;')
    print('        }')
    print('        free(decryptedName);')
    print('    }')
